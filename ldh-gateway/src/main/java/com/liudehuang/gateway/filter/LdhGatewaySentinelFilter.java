...
import com.liudehuang.gateway.fallback.LdhGatewayBlockFallbackProvider;
...
@Slf4j
@Configuration
public class LdhGatewaySentinelFilter {
    @Bean
    ...
    @PostConstruct
    public void doInit() {
        ZuulBlockFallbackManager.registerProvider(new LdhGatewayBlockFallbackProvider());
        initGatewayRules();
    }
    ...
    private void initGatewayRules() {
         ...
         * 其中的字段：
         *      1.parseStrategy：从请求中提取参数的策略，目前支持提取来源 IP（PARAM_PARSE_STRATEGY_CLIENT_IP）、
         ...
         *      3.pattern：参数值的匹配模式，只有匹配该模式的请求属性值会纳入统计和流控；若为空则统计该请求属性的所有值。
         *      （1.6.2 版本开始支持）
         *      4.matchStrategy：参数值的匹配策略，目前支持精确匹配（PARAM_MATCH_STRATEGY_EXACT）、
